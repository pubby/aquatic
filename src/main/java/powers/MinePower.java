...
public class MinePower extends AbstractAquaticPower {
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.type == DamageInfo.DamageType.NORMAL && info.owner != null && info.owner != owner) {
            this.flash();
            this.addToTop(new DamageAction(owner, new DamageInfo(owner, amount, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.FIRE, true));
            this.addToTop(new DamageAction(info.owner, new DamageInfo(info.owner, backfireAmount(), DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.FIRE, true));
