...
public class RenderPatch
{
    ...
    {
        if (__instance.isDying || __instance.isEscaping || AbstractDungeon.getCurrRoom().phase != AbstractRoom.RoomPhase.COMBAT 
        || AbstractDungeon.player.isDead || Settings.hideCombatElements) {
            return;
        }

        if (__instance.hasPower(MinePower.POWER_ID) && !AbstractDungeon.player.hasRelic(OvenMitt.ID) && !AbstractDungeon.player.hasPower(DetonatePower.POWER_ID)) {
